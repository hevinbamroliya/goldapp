...
frappe.provide("erpnext.utils.BarcodeScanner");
...
// 				() => this.set_custom_metal_type(row, custom_metal_type),
// 				() => this.set_custom_purity(row, custom_purity),
// 				// Other function calls...
// 			]);
// 		});
// 	}
	
// 	async set_item(row, item_code, barcode, batch_no, serial_no, custom_metal_type) {
// 		return new Promise(resolve => {
// 			const increment = async (value = 1) => {
// 				const item_data = {item_code: item_code};
// 				item_data[this.qty_field] = Number((row[this.qty_field] || 0)) + Number(value);
// 				item_data[this.custom_metal_type_field] = custom_metal_type;
// 				item_data[this.custom_metal_type_field] = custom_metal_type; // Set custom_metal_type
...
// 	set_custom_metal_type(row, custom_metal_type) {
// 		return new Promise(resolve => {
// 			if (custom_metal_type && frappe.meta.has_field(row.doctype, this.custom_metal_type_field)) {
// 				frappe.model.set_value(row.doctype, row.name, this.custom_metal_type_field, custom_metal_type)
// 					.then(() => resolve());
// 			} else {
// 				resolve();
// 			}
...
// 	set_custom_purity(row, custom_purity) {
// 		return new Promise(resolve => {
// 			if (custom_purity && frappe.meta.has_field(row.doctype, "custom_purity")) {
// 				frappe.model.set_value(row.doctype, row.name, "custom_purity", custom_purity)
// 					.then(() => resolve());
// 			} else {
// 				resolve();
// 			}
...
// }	
	...
	constructor(opts) {
		...
		this.custom_metal_type_field = opts.custom_metal_type || "custom_metal_type";
		this.custom_purity_field = opts.custom_purity || "custom_purity";
	...
	update_table(data) {
		return new Promise(resolve => {
			...
			const {item_code, barcode, batch_no, serial_no, uom, custom_metal_type, custom_purity} = data;

			let row = this.get_row_to_modify_on_scan(item_code, batch_no, uom, barcode, custom_metal_type, custom_purity);
			...
			frappe.run_serially([
				...
				() => this.set_item(row, item_code, barcode, batch_no, serial_no, custom_metal_type, custom_purity).then(qty => {
	...
	set_item(row, item_code, barcode, batch_no, serial_no, custom_metal_type,custom_purity) {
		return new Promise(resolve => {
			const increment = async (value = 1) => {
				const item_data = {item_code: item_code,};
				frappe.flags.trigger_from_barcode_scanner = true;
				item_data[this.qty_field] = Number((row[this.qty_field] || 0)) + Number(value);
				item_data[this.custom_metal_type_field] = custom_metal_type; //custom metal type
				item_data[this.custom_purity_field] = custom_purity; //custom purity 
				await frappe.model.set_value(row.doctype, row.name, item_data);
				return value;
			};
   ...
   /////-----set costom metal type-----/////
	...
	set_custom_purity(row, custom_purity) {
		return new Promise(resolve => {
			if (custom_purity && frappe.meta.has_field(row.doctype, this.custom_purity_field)) {
				frappe.model.set_value(row.doctype, row.name, this.custom_purity_field, custom_purity)
					.then(() => resolve());
			} else {
				resolve();
			}
